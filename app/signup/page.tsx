...
export default function Signup() {
  ...
  const handleGoogleLoginSuccess = async (credentialResponse: any) => {
    ...
    try {
      ...
      toast.success('User registered successfully!', {
        ...
        position: 'top-right',
        style: {
          background: '#4caf50', // Success toast background color
          color: '#fff',         // Success toast text color
        },
      });
  ...
  const handleGoogleLogin = useGoogleLogin({
    onSuccess: (response) => {
      ...
      handleGoogleLoginSuccess(response); // Pass the response to the success handler
    },
    onError: () => {
